...
| `test_screen_count_rows.mjs` | Per-screen lists render one row per screen — runs the real renderer and counts |
| `test_tab_scope.mjs` | The screen picker is hidden on bridge-global tabs — and only on those |
